...
(function () {
  ...
  window.FreezeUI = function () {
    ...
    setTimeout(function () {
      ...
      if(!options.element){
        freezeHtml.style.position = "fixed";
      }
  ...
  window.UnFreezeUI = function () {
    ...
    setTimeout(function () {
      let _freezeHtml = null;

      if (options.element) {
        _freezeHtml = options.element.querySelector(".freeze-ui");
      } else {
        _freezeHtml = document.querySelector(".freeze-ui");
      }

      if (_freezeHtml) {
        _freezeHtml.classList.remove("is-unfreezing");

        if (_freezeHtml.parentElement) {
          _freezeHtml.parentElement.removeChild(_freezeHtml);
        }
      }
    }, delay);
  };
